...
app = Flask(__name__)
cors = CORS(app)
app.config['MONGO_URI'] = 'mongodb+srv://Wolfteam111:Wolfteam111@atlascluster.sfflokd.mongodb.net/mytable?retryWrites=true&w=majority'
...
def retrieveFromName(name):
    currentCollection = mongo.db.mytable
    data = currentCollection.find_one({"username" : name})
    return jsonify({'username' : data['username'], 'email' : data['email'], 'password' : data['password'], 'quote' : data['quote']})
